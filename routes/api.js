...
// problems would be the first to break. Rather than adopting Anthropic's native
// compaction (which expects the caller to persist and replay opaque content blocks
// across an ongoing in-memory conversation — a poor fit here, since each request
...
async function maybeCompact(threadId, allTurns, existingSummary, coveredThroughId) {
  ...
  try {
    ...
    await pool.query(
      `UPDATE threads SET compacted_summary = $1, compacted_through_id = $2 WHERE id = $3`,
      [summary, toFold[toFold.length - 1].id, threadId]
    );
    return { summary, recentTurns };
  } catch (err) {
    console.error('[api] compaction error, proceeding uncompacted:', err);
    return { summary: existingSummary, recentTurns: uncovered };
  }
}
...
router.post('/threads/:id/messages', asyncRoute(async (req, res) => {
  ...
  const [priorTurns, threadRow] = await Promise.all([
    pool.query(`SELECT id, role, content, attachment_label FROM turns WHERE thread_id = $1 ORDER BY id ASC`, [threadId]),
    pool.query(`SELECT compacted_summary, compacted_through_id FROM threads WHERE id = $1`, [threadId]),
  ]);
