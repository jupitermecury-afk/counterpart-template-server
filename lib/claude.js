...
- flag_verification — the moment you make a claim that rests on general knowledge you cannot confirm —
  regulatory specifics, current pricing, institution-specific policies, third-party practices — call this
  with the claim itself and, whenever you can name one, what would actually confirm it (which office to
  check, what to call, what to look up), in addition to naming it in your prose. This is what keeps the
  verification register honest and actionable, not just a list of doubts.
- resolve_verification — when the person tells you the real answer to something you flagged, or you
  otherwise actually learn it, close the loop yourself — call this rather than leaving it open for the
  person to update by hand. You were given the current verification register as context; match its exact
  wording.
- update_held_thread — when you identify something the person needs to provide or do, or the shape of
  what you're holding for them changes (something opens, something settles, something closes), call this
  so the held record stays accurate without the person having to manage it. Write each thing as a
  discrete item, named specifically enough that it's obvious when it's done — never a summary paragraph
  folding several things together. When the person provides or does what an item was asking for, update
  that same item (matching its exact text) to reflect that rather than leaving it stale. When a held
  thing has a real deadline or window, set due_at. When it's stalled on something specific — a person's
  signature, another party's response, a waiting period — set blocked_on.
...
You also have a real web search tool. Use it whenever an answer depends on something that could have
changed since your training — current prices, fees, phone numbers, addresses, government requirements,
exchange rates, company policies, recent news, or anything the person flags as time-sensitive. This is a
genuine search, not a guess dressed up as one — search rather than answer from memory when the person
needs the current, real number. You don't need to hide that you searched; naming a source when it's
useful is fine. If you already searched something earlier in this same conversation, use what you
already found — searching the same fact again wastes the room you have for the actual work.

When a person has explicitly asked you for a real artifact — especially if an earlier attempt in this
same conversation already fell short or cut off before producing it — get to the artifact call directly.
A short line acknowledging what you're doing is enough; you do not need fresh preamble, re-verified
context, or a restated summary of the situation before every attempt. The room you have is finite in any
single reply — spend it on the thing they are actually waiting for, not on re-establishing context you
already have.
...
const TOOLS = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        confirm_via: { type: 'string', description: 'What would actually confirm this claim — which office, site, call, or lookup. Omit only if you genuinely cannot name one.' },
      },
      required: ['claim_text'],
    },
  },
  {
    ...
    description: 'Mark a previously flagged claim as confirmed or otherwise resolved, now that you actually have real information about it — e.g. the person told you the real answer, or you looked it up. Match claim_text exactly to an entry in the current verification register you were given as context.',
    input_schema: {
      type: 'object',
      properties: {
        claim_text: { type: 'string', description: 'The exact claim_text of the existing verification register item this resolves.' },
        status: { type: 'string', enum: ['person_confirming', 'counterpart_checking', 'proceeding_unconfirmed', 'confirmed'] },
      },
      required: ['claim_text', 'status'],
    },
  },
  {
    name: 'update_held_thread',
    description: 'Update what is being held for the person about this problem thread — including when it needs attention and what it is actually waiting on.',
    input_schema: {
      type: 'object',
      properties: {
        text: { type: 'string' },
        ...
        meta: { type: 'string' },
        due_at: { type: 'string', description: 'ISO 8601 date/time this item genuinely needs attention by — a real deadline, court date, or window closing. Omit if there is no real time pressure.' },
        blocked_on: { type: 'string', description: 'What this is actually waiting on right now — e.g. "landlord\'s response", "person to sign and mail the form", "the 30-day notice period". Omit if nothing is blocking it.' },
      },
      required: ['text', 'glyph'],
    },
  },
...
// Anthropic's hosted web search tool — the model decides when to call it, mid-reasoning,
// same as any other tool; results resolve server-side within the response. Replaces the old
// silent pre-flight Tavily classifier (removed below) — that approach never told the model
// it had search, so it couldn't reason about when to use it, and it ran a hidden heuristic
// before every message whether or not search was actually needed.
//
// Deliberately the basic (non-dynamic-filtering) variant: web_search_20260209 auto-injects
// its own internal code_execution tool to filter results, which collides with the
// code_execution tool we already declare above for genuine computation — the API rejects
// the request outright ("Auto-injecting tools would conflict with existing tool names").
...
const REQUEST_TOOLS = [...TOOLS, CODE_EXECUTION_TOOL, WEB_SEARCH_TOOL, ...MCP_TOOLSET_TOOLS];
...
async function streamCounterpartReply({ model, maxTokens, messages, onText, onTool, onServerToolEvent, extraSystemContext }) {
  ...
  const system = systemPrompt() + (extraSystemContext || '');
